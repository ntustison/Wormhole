...
my $baseDirectory = "/Users/ntustison/Data/WMH/ADNI/";
...
for( my $i = 0; $i < @subjectDirs; $i++ )
  ...
  my $flairxt1 = "${outputPrefix}_ants_FLAIRxT1.nii.gz";
  my $sysu1 = "${outputPrefix}_ants_FLAIR_SysuWmhSegmentation.nii.gz";
  my $sysu2 = "${outputPrefix}_ants_FLAIR_SysuWmhAxialSegmentation.nii.gz";
  my $ew1 = "${outputPrefix}_ants_FLAIR_EwDavidOctantWmhSegmentation.nii.gz";
  my $ew2 = "${outputPrefix}_ants_FLAIR_EwDavidSlicewiseWmhSegmentation_300mb.nii.gz";
  my $ew3 = "${outputPrefix}_ants_FLAIR_EwDavidSlicewiseWmhSegmentation_7mb.nii.gz";
...
#  if( -e $flairxt1 && ( ! -e $sysu1 || ! -e $sysu2 || ! -e $ew1 || ! -e $ew2 ) )
  if( -e $flairxt1 && ! -e $ew2 )
    {
    print FILE "python3 ${baseDirectory}/../Scripts/do_ants_processing.py $flairxt1 $flair $outputPrefix $numberOfThreads\n";
    $doCluster = 1;
    }
