...
if( doBinaryClassification == FALSE )
  ...
  for( i in seq.int( length( algorithms ) ) )
    ...
    dxFormula <- as.formula( paste0( "Diagnosis ~ TotalWmhVolume + ", paste( covariates, collapse = "+" ) ) ) 
    ...
    for( j in seq.int( numberOfRuns ) )
      ...
      predictions$observed <- perAlgorithmDataFrame$Diagnosis[-trainingIndices]
    ...
    ggsave( paste0( figuresDirectory, "wmhDxConfusionMatrix_", algorithms[i], ".pdf" ), confusionPlot )    
  ...
  ggsave( paste0( figuresDirectory, "wmhDxRoc.pdf" ), g, width = 5, height = 5, units = "in" )
